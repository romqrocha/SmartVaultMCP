...
if (pages == "") {
  ...
  if (typeof pages.startsWith === 'function' && pages.startsWith("dv.pages")) {
...
if (!taskFiles) {
  taskFiles = [
    ...new Set(
      dv.pages().file.map((f) => f.tasks.filter((t) => !t.completed)).path
    ),
  ].sort();
} else {
  taskFiles = [...new Set(dv.pagePaths(taskFiles))].sort();
}
if (!options) {
  options = "";
}
...
if (css) {
  ...
  style.innerHTML = css;
  rootNode.querySelector("span").append(style);
}

// Icons
var doneIcon =
  '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z"></path><path d="M7.5 12.5L10.5 15.5L16 10"></path></svg>';
var dueIcon =
...
var overdueIcon =
  '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>';
var processIcon =
  '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6H5a2 2 0 0 0-2 2v3a2 2 0 0 0 2 2h13l4-3.5L18 6Z"></path><path d="M12 13v9"></path><path d="M12 2v4"></path></svg>';
var dailynoteIcon =
...
var tagIcon =
  '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2H2v10l9.29 9.29c.94.94 2.48.94 3.42 0l6.58-6.58c.94-.94.94-2.48 0-3.42L12 2Z"></path><path d="M7 7h.01"></path></svg>';
var repeatIcon =
...
var forwardIcon =
  '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 17 20 12 15 7"></polyline><path d="M4 18v-2a4 4 0 0 1 4-4h12"></path></svg>';
var alertIcon =
...
function getMeta(tasks) {
  for (i = 0; i < tasks.length; i++) {
    ...
    if (
      ...
      if (!dailyTaskMatch) {
        if (
          moment(dailyNoteMatch[1], dailyNoteFormat).format("YYYY-MM-DD") <
          today
        ) {
          if (forward == true) {
            timelineDates.push(today);
      ...
      moment(dailyNoteMatch[1], dailyNoteFormat).format("YYYY-MM-DD") >= today
    ) {
      timelineDates.push(
        moment(dailyNoteMatch[1], dailyNoteFormat).format("YYYY-MM-DD")
      );
      happens["cancelled"] = moment(dailyNoteMatch[1], dailyNoteFormat).format(
        "YYYY-MM-DD"
      );
    ...
    while ((outerLink = /\[([^\]]+)\]\(([^)]+)\)/g.exec(tasks[i].text))) {
      tasks[i].text = tasks[i].text.replace(
        outerLink[0],
        "<a class='external-link outerLink' href='" +
          outerLink[2] +
          "'>" +
          outerLink[1] +
          "</a>"
    ...
    while ((innerLink = /\[\[([^\]]+)\]\]/g.exec(tasks[i].text))) {
      tasks[i].text = tasks[i].text.replace(
        innerLink[0],
        "<a class='internal-link innerLink' href='" +
          innerLink[1] +
          "'>" +
          innerLink[1] +
          "</a>"
    ...
    // Reminder Syntax
    var reminderMatch = taskText.match(
...
function getSelectOptions() {
  ...
  const currentDailyNote =
    dailyNoteFolder + moment().format(dailyNoteFormat) + ".md";
  taskFiles.push(currentDailyNote);
  if (inbox) {
    taskFiles.push(inbox);
  }
  ...
  taskFiles.forEach(function (file) {
    ...
    opt.value = file;
    var secondParentFolder =
      file.split("/")[file.split("/").length - 3] == null ? "" : "… / ";
    var parentFolder =
      file.split("/")[file.split("/").length - 2] == null
        ? ""
        : secondParentFolder +
          "📂&nbsp;" +
          file.split("/")[file.split("/").length - 2] +
          " / ";
...
function setEvents() {
  rootNode.querySelectorAll(".counter").forEach((cnt) =>
    cnt.addEventListener("click", () => {
      var activeFocus = Array.from(rootNode.classList).filter(
        (c) => c.endsWith("Filter") && !c.startsWith("today")
      );
      ...
      rootNode.classList.remove.apply(
        rootNode.classList,
        Array.from(rootNode.classList).filter(
          (c) => c.endsWith("Filter") && !c.startsWith("today")
        )
      );
  ...
  rootNode.querySelectorAll(".task:not(.star, .add)").forEach((t) =>
  ...
  rootNode.querySelector(".newTask").addEventListener("input", () => {
    ...
    if (newTask.includes("due ")) {
      input.value = newTask.replace("due", "📅");
    }
    if (newTask.includes("start ")) {
      input.value = newTask.replace("start", "🛫");
    }
    if (newTask.includes("scheduled ")) {
      input.value = newTask.replace("scheduled", "⏳");
    }
    if (newTask.includes("done ")) {
      input.value = newTask.replace("done", "✅");
    }
    if (newTask.includes("high ")) {
      input.value = newTask.replace("high", "⏫");
    }
    if (newTask.includes("medium ")) {
      input.value = newTask.replace("medium", "🔼");
    }
    if (newTask.includes("low ")) {
      input.value = newTask.replace("low", "🔽");
    }
    if (newTask.includes("repeat ")) {
      input.value = newTask.replace("repeat", "🔁");
    }
    if (newTask.includes("recurring ")) {
      input.value = newTask.replace("recurring", "🔁");
    }

    // Dates
    if (newTask.includes("today ")) {
      input.value = newTask.replace("today", moment().format("YYYY-MM-DD"));
    }
    if (newTask.includes("tomorrow ")) {
      input.value = newTask.replace(
        "tomorrow",
    ...
    var futureDate = newTask.match(
      /(in)\W(\d{1,3})\W(days|day|weeks|week|month|years|year) /
    );
    if (futureDate) {
    ...
    if (weekday) {
      ...
      if (moment().isoWeekday() < dayINeed) {
        input.value = newTask.replace(
          weekday[1],
          moment().isoWeekday(dayINeed).format("YYYY-MM-DD")
        );
      } else {
        input.value = newTask.replace(
          weekday[1],
          moment().add(1, "weeks").isoWeekday(dayINeed).format("YYYY-MM-DD")
        );
      }
    }
    ...
    if (line && col) {
      try {
        ...
        cmEditor.setSelection(
          { line: parseInt(line), ch: 6 },
          { line: parseInt(line), ch: parseInt(col) }
        );
        cmEditor.focus();
      } catch (err) {
        new Notice("Something went wrong!");
  ...
  app.workspace.openLinkText("", link).then(() => {
    if (line && col) {
      try {
        ...
        cmEditor.setSelection(
          { line: parseInt(line) + addRange, ch: 6 },
          { line: parseInt(line) + addRange, ch: parseInt(col) + 13 }
        );
        cmEditor.focus();
      } catch (err) {
        new Notice("Something went wrong!");
...
function getTimeline(tasks) {
  ...
  for (i = 0; i < timelineDates.length; i++) {
    ...
    var tasksFiltered = tasks
      .filter((t) =>
        Object.values(t.happens).includes(timelineDates[i].toString())
      )
      .sort(eval(sort));
    ...
    if (year != lastYear) {
      ...
      if (moment().format("YYYY") == year) {
        yearNode.classList.add("current");
      }
    ...
    if (timelineDates[i] == today) {
      ...
      var scheduledCount = tasksFiltered.filter(
        (t) => t.happens["scheduled"]
      ).length;
      var doneCount = tasksFiltered.filter((t) => t.happens["done"]).length;
      var dailynoteCount = tasksFiltered.filter(
        (t) => t.happens["dailynote"]
      ).length;
      var processCount = tasksFiltered.filter(
        (t) => t.happens["process"]
      ).length;
      var todoCount = tasksFiltered.filter(
        (t) => !t.completed && !t.happens["overdue"] && !t.happens["unplanned"]
      ).length;
      ...
      // Counter
      var todayContent =
        "<div class='todayHeader' aria-label='Focus today'>Today</div>";
      todayContent += "<div class='counters'>";
      todayContent +=
        "<div class='counter' id='todo' aria-label='Filter tasks to do'><div class='count'>" +
        todoCount +
        "</div><div class='label'>To Do</div></div>";
      todayContent +=
        "<div class='counter' id='overdue' aria-label='Filter overdue tasks'><div class='count'>" +
        overdueCount +
        "</div><div class='label'>Overdue</div></div>";
      todayContent +=
        "<div class='counter' id='unplanned' aria-label='Filter unplanned tasks'><div class='count'>" +
        unplannedCount +
        "</div><div class='label'>Unplanned</div></div>";
      todayContent += "</div>";
      // Quick Entry panel
      ...
      todayContent +=
        "<div class='right'><button class='ok' aria-label='Append new task to selected note'>";
      todayContent +=
    ...
    tasksFiltered.forEach(function (item) {
      ...
      var cls = Object.keys(item.happens)
        .find((key) => item.happens[key] === timelineDates[i].toString())
        .replace("Forward", "");
      ...
      if (item.repeat) {
        info +=
          "<div class='repeat' aria-label=''><div class='icon'>" +
          repeatIcon +
          "</div><div class='label'>" +
          item.repeat.replace("🔁", "") +
          "</div></div>";
      ...
      if (item.priorityLabel) {
        info +=
          "<div class='priority' aria-label=''><div class='icon'>" +
          priorityIcon +
          "</div><div class='label'>" +
          item.priorityLabel +
          "</div></div>";
      ...
      var task =
        ...
        "' aria-label='" +
        file +
        "'><div class='timeline'><div class='icon'>" +
        icon +
        "</div><div class='stripe'></div></div><div class='lines'><a class='internal-link' href='" +
        link +
        "'><div class='content'>" +
        text +
        "</div></a><div class='line info'>" +
        info +
        "</div></div></div>";
      content += task;
    });
    ...
    // Set Date Template
    var date =
      "<div class='dateLine'><div class='date'>" +
      date +
      "</div><div class='weekday'>" +
    ...
    rootNode
      ...
      .appendChild(
        dv.el("div", date, {
          cls: "details " + detailsCls,
          attr: {
            "data-year": year,
            "data-types": containedTypesPerDay.join(" "),
          },
        })
      );
